classdef TimeVaringConnectivity < NeuroMethod & NeuroPlot.NeuroPlot & NeuroResult
    ...
    methods
        function obj=TimeVaringConnectivity(varargin)
            if nargin==1
                data=varargin{1};
                for i=1:length(data)
        ...
        function obj = getParams(obj)
            methodlist={'Coherence in chronux','Partial Directed coherence(eMVAR)','SIFT toolbox in EEGlab','Connectivity in fieldtrip'};
            method=listdlg('PromptString','Select the Connectivity method','ListString',methodlist);
            switch method
                 case 1
                     obj.Params=ChronuxFcn.getparams;
                case 2
    ...
    methods(Static)
         ...
         function neuroresult=recal(params,neuroresult,resultname)
            ...
            % the connectivity value is a 4-D {event} channel*channel*t*f matrix 
            if isprop(neuroresult,'TimeVaringConnectivity')
                currentname=neuroresult.TimeVaringConnectivity.getTaginfo('Tagvalue','fileTag');
                if contains(resultname,currentname)
                    warning([resultname,'is in the current result, skip.']);
                    return;
                end
            end
            obj=TimeVaringConnectivity();
            obj.Params=params;
