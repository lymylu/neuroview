...
%                     eeglab;
         ...
         function obj=GenerateObjects(obj,filemat)
             ...
             set(Figpanel1,'SelectionChangedFcn',@(~,src) obj.getResulttype(FilePath,'switch'));
             NeuroPlot.commandcontrol('Parent',Figcontrol1,'Plottype','imagesc','Command','create');
             ...
             FigurecommandPanel=uix.HBox('Parent',obj.FigurePanel,'Tag','Basecorrect','Padding',5);
        ...
        function obj=Changefilemat(obj,filemat,varargin)
             ...
             global Resultorigin Eventpanel Channelpanel Eventdescription FilePath Channeldescription matvalue Blacklist Channellist Eventlist err      
             tmpobj=findobj(obj.NP,'Tag','Matfilename');
             ...
             close(h);
             obj.getResulttype(FilePath,'loading'); 
             Figpanel1=findobj('Parent',obj.FigurePanel,'Tag','Figpanel1');
             set(Figpanel1,'SelectionChangedFcn',@(~,src) obj.getResulttype(FilePath,'switch'));
             ...
             Channelpanel=Channelpanel.assign('liststring',Channellist,'listtag',{'ChannelfromIndex','ChanneltoIndex'},'typetag',{'Channelfromtype','Channeltotype'},'typestring',Channeldescription,'blacklist',Blacklist(matvalue).Channelindex);
             if nargin>2
        ...
        function getResulttype(obj,FilePath,option)
            global ResultCon tmp_t tmp_f origin_t
            switch option
                case 'loading'
                    Resultname=fieldnames(FilePath.Result);
                    TabFigure=findobj(obj.NP,'Parent',obj.FigurePanel,'Tag','Figpanel1');
                    Figcontrol1=findobj(obj.NP,'Tag','Figcontrol1');
                    TabTitle=[];                  
                    for i=1:length(Resultname)
                        if ~strcmp(Resultname{i},'origin')&& isempty(findobj('Parent',TabFigure,'Tag',Resultname{i}))
                    ...
                    ResultCon=getfield(FilePath.Result,TabFigure.TabTitles{1});
                    constant=FilePath.Constant;
                    tmp_f=eval(['constant.',TabFigure.TabTitles{1},'.f;']);
                    tmp_t=eval(['constant.',TabFigure.TabTitles{1},'.t;']);
                    origin_t=constant.origin.t;
                case 'switch'
                    TabFigure=findobj(obj.NP,'Parent',obj.FigurePanel,'Tag','Figpanel1');
                    Figcontrol1=findobj(obj.NP,'Tag','Figcontrol1');
                    n=TabFigure.Selection;
                    obj.ChangeLinkedCommand(Figcontrol1,TabFigure);
                    ResultCon=getfield(FilePath.Result,TabFigure.TabTitles{n});
                    constant=FilePath.Constant;
                    tmp_f=eval(['constant.',TabFigure.TabTitles{n},'.f;']);
                    tmp_t=eval(['constant.',TabFigure.TabTitles{n},'.t;']);
            end
        end
        function ChangeLinkedCommand(obj,control,panel)
            tmpobj=findobj(obj.NP,'Parent',panel);          
            NeuroPlot.commandcontrol('Parent',control,'Plottype','imagesc','Command','changelinkedaxes','Linkedaxes',tmpobj(panel.Selection));
            try
    ...
    methods (Access='private')
         function Resultplotfcn(obj)
            ...
            tmppanel=findobj(obj.NP,'Tag','Figpanel1');  
            tmpobj=findobj(obj.NP,'Parent',tmppanel); 
            delete(findobj('Parent',tmpobj(tmppanel.Selection),'Type','axes'));
            figaxes=axes('Parent',tmpobj(tmppanel.Selection));
            imagesc(tmp_t,tmp_f,tmpdata');
            tmpcontrol=findobj(obj.NP,'Tag','Figcontrol1');
            figaxes.XLim=[min(tmp_t),max(tmp_t)];
            figaxes.YLim=[min(tmp_f),max(tmp_f)];
            figaxes.YDir='normal';
             obj.ChangeLinkedCommand(tmpcontrol,tmppanel);
            Resultoriginto=Resultorigin(:,ismember(Channellist,channellistto.String(channellistto.Value)),...
                ismember(Eventlist,eventlist.String(eventlist.Value)));
            ...
            plot(origin_t,cat(2,tmpdata2,tmpdata1));
            figaxes.XLim=[min(origin_t),max(origin_t)];
            tmpcontrol=findobj(obj.NP,'Tag','Figcontrol2');
            NeuroPlot.commandcontrol('Parent',tmpcontrol,'Command','assign','linkedaxes',tmpobj);
            tmpobj=findobj(obj.NP,'Tag','Savename');
            tmpobj1=findobj(obj.NP,'Tag','Eventtype');
            tmpobj2=findobj(obj.NP,'Tag','Channelfromtype');
            tmpobj3=findobj(obj.NP,'Tag','Channeltotype');
