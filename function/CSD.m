...
%     xAxis = [SPm:SPm:SPm*length(data)]'; % x-axis for plots
    xAxis=linspace(p.Results.timeaxis(1),p.Results.timeaxis(2),length(data));
...
%         figure(1); % plots voltage data for the 2nd to 2nd last electrode
        ...
        axis([p.Results.timeaxis(1),p.Results.timeaxis(2),MmV*0.5,(MmV*(size(data,2)-1))-(MmV*0.5)]);        
...
%         figure(1) % plots the voltage data for all electrodes
    ...
    % figure labels 
    ylabel('Electrode');
    title({'Voltage'; ['scale bar = ' num2str(scale1)...
        ' V)']});
    xlabel('Time (ms)');
    ...
    if CSDtype == 1; % plots the CSD for the standard CSD method 
        ...
        axis([p.Results.timeaxis(1),p.Results.timeaxis(2),MC*0.5,(MC*(size(CSD,2)-1))-(MC*0.5)]);
    ...
    % figure labels
    ylabel('Electrode')
    title({'CSD'; ['(scale bar = ' num2str(scale2) ...
        ' ' unitsCurrent '/' unitsLength '^{3})']});
    xlabel('Time (ms)')
    % plots color bar then turns visbility off. Allows all subplots to be
    % aligned vertically (right subplot contains colorbar)
    colorbar('SouthOutside','Box','off','Visible','off')
...
%     figure(1)
    ...
    set(im, 'XData', xAxis);
    axis([p.Results.timeaxis(1),p.Results.timeaxis(2),-inf,inf]);
    ylabel('Electrode');
    xlabel('Time (ms)');
