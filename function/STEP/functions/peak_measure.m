function [ST] = peak_measure(EEG,ST)


if ST.method_type==5  
    T=find(ST.f_T*1000>=ST.interval(1)&ST.f_T*1000<=ST.interval(2));
    dat=squeeze(ST.f_trials(T,:));
    for i=1:size(ST.peak,1)
        T_peak(i)=find(abs(ST.f_T*1000-ST.peak(i,1))==min(abs(ST.f_T*1000-ST.peak(i,1))))-T(i)+1;        
    end
    t=ST.f_T(T);
else
    T=find(EEG.times>=ST.interval(1)&EEG.times<=ST.interval(2));
    dat=squeeze(EEG.data(ST.channel_index,T,:));
    for i=1:size(ST.peak,1)
        T_peak(i)=find(abs(EEG.times-ST.peak(i,1))==min(abs(EEG.times-ST.peak(i,1))))-T(i)+1;
    end
    t=EEG.times(T);
end
...
for i=1:size(Fits,2)
    ...
    for ii=1:length(V_peak1)
        Duration=round(EEG.srate*ST.peak_interval(ii)/1000);
