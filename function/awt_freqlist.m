...
% 3/16/2011: vectorized by zifang zhao,tested with 'Gabor', the other mode
% is unknown yet
n = length(x);
sigma2 = 1;
x = x(:);
...
% s_array=s_array(s_array(:)'>=minscale & s_array(:)'<=maxscale);
%vectorized version
switch(type)
    case 'Gabor'
    ...
    case 'Sombrero'
        ...
        Psi = bsxfun(@times,-realpow(pi*sigma2^5,1/4)*sqrt(8/3)*realpow(freq,2).*exp(-sigma2/2*realpow(freq,2)),sqrt(s_array));
        Psi = Psi .* (omega>0);
end;
...
%             freq =  (s .* omega  - xi);
% %             Psi = realpow(4.*pi.*sigma2,1/4)*sqrt(s) *exp(-sigma2/2*freq.*freq);
%             Psi = realpow(4.*pi.*sigma2,1/4)*sqrt(s) *exp(-sigma2/2*realpow(freq,2));
%         %    Psi = Psi .* (omega>0);
%         case 'Lusin1'
%             % to be completed
%         case 'Sombrero'
%             freq = s.*omega;
% %             Psi = -realpow(pi*sigma2^5,1/4)*sqrt(8/3)*freq.^2.*sqrt(s).*exp(-sigma2/2*freq.*freq);
%             Psi = -realpow(pi*sigma2^5,1/4)*sqrt(8/3)*freq.^2.*sqrt(s).*exp(-sigma2/2*realpow(freq,2));
%             Psi = Psi .* (omega>0);
%     end;
% 
