...
spk_time=double(spk_time)/str2num(Spikeoutput.Fs);
...
for i=1:length(clusternumber)
    if logical(sum(ismember(channel,channel_map(channel_shanks==clusternumber(i))+1)))
        clustername=cluster_info((cluster_info(:,shank_index)==clusternumber)&strcmp(raw(:,group_index),'good'),id);
        for j=1:length(clustername)
            tmpspikename=['cluster',num2str(clusternumber),'_',num2str(clustername(j))];
