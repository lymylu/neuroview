classdef NeuroDatanew < BasicTag & dynamicprops
    ...
    % Current Supprot data type, NeuroViewdata (legendecy, includ LFPdata, SPKdata, EVTdata, Videodata), NWB
...
%         Videodata=[];
    ...
    methods (Access='public')     
        function obj = fileappend(obj, filepath)
            ...
            select=listdlg("PromptString",'Select the data type in the subject directory','SelectionMode','single','ListString',{'NeuroData','NWB'});
            switch select
                case 1 % NeuroViewdata legendecy.
                    obj.initialize('Neuroview');
                case 2 % Neurodata without borders.
                    obj.initialize('NWB');
            end
        end
        function obj=initialize(obj,type)
            addprop(obj,'Datatype');
            switch type
                case 'Neuroview'
                    obj.Datatype='Neuroview';
                    addprop(obj,'LFPdata'); obj.LFPdata=LFPData();
                    addprop(obj,'SPKdata'); obj.SPKdata=SPKData();
                    addprop(obj,'EVTdata'); obj.EVTdata=EVTData();
                    addprop(obj,'Videodata'); obj.Videodata=VideoData();
                    figure;
                    panels=uix.Panel('Parent',gcf,'Title','information of the Neuroview data');
                    panel=uix.VBoxFlex('Parent',panels);
                    uicontrol('Parent',panel,'Style','pushbutton','String','Select LFP file','Callback',obj.LFPdata.fileappend);
                    uicontrol('Parent',panel,'Style','text','String','SPKdata');
                    uicontrol('Parent',panel,'Style','pushbutton','String','Select SPKsorting data','Callback',obj.SPKdata.fileappend);
                    uicontrol('Parent',panel,'Style','pushbutton','String','Select EVT file','Callback',obj.EVTdata.fileappend);
            end
        end
